...
def formatfile(fpath):
    try:
        subprocess.run(
            ["npx", "prettier@3.4.2", "--write", fpath],
            check=True,
            capture_output=True,
            text=True,
            shell=True
...
def countdays(inputfile,outputfile,day):
    ...
    with open(inputfile, "r", encoding="utf-8") as file:
        for line in file:
            ...
            try:
                dt = None
                if re.match(r"\d{4}/\d{2}/\d{2} \d{2}:\d{2}:\d{2}", line):  # yyyy/mm/dd HH:MM:SS
                    dt = datetime.strptime(line, "%Y/%m/%d %H:%M:%S")
                elif re.match(r"\d{4}-\d{2}-\d{2}", line):  # yyyy-mm-dd
                    dt = datetime.strptime(line, "%Y-%m-%d")
                elif re.match(r"\d{2}-[A-Za-z]{3}-\d{4}", line):  # dd-MMM-yyyy
                    dt = datetime.strptime(line, "%d-%b-%Y")
                elif re.match(r"[A-Za-z]{3} \d{2}, \d{4}", line):  # MMM dd, yyyy
...
def uvtask(url,email):
    ...
    subprocess.run(["uv", "run", "python", "datagen.py", email], shell=True, check=True)
    file_path = "/data"  
    ...
    if "choices" in result and result["choices"]:
        ...
        if function_call:
            ...
            if isinstance(arguments, str):
                try:
                    arguments = json.loads(arguments)  # Convert JSON string to dictionary
